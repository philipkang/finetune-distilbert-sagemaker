...
def calculate_accu(big_idx,targets):
    ...
    [0.2,0.1,0.7,0.55] # Breakthrough in cancer vaccine 0 0 1 0    target 0 0 1 0
...
def train(epoch,model,device,training_loader,optimizer,loss_function):
    ...
    for _, data in enumerate(training_loader,0):
        ...
        if _ %5000 == 0: #100K / 4 = 25000 steps
...
def valid(epoch, model, testing_loader, device, loss_function):
    ...
    for _, data in enumerate(testing_loader,0):
        ...
        print(f"Validation accuracy per Epoch: {epoch_accu} at epoch {epoch}")
